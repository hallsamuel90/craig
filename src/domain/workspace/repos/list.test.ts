...
import type { CraigIndex, RepoRecord } from "../types.js";
...
const noopEnsure = async (): Promise<CraigIndex> => ({} as CraigIndex);

describe("listRegisteredRepos", () => {
  test("returns all repos", async () => {
    const repos = [makeRepo("repo_a"), makeRepo("repo_b")];
    const deps = { listRepos: async () => repos, ensureCraigState: noopEnsure };
  ...
  test("returns empty repos list when none exist", async () => {
    const deps = { listRepos: async () => [], ensureCraigState: noopEnsure };
    const result = await listRegisteredRepos(makePaths(), deps);
