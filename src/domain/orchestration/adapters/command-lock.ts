...
export async function withCommandStoreLock<T>(paths: CraigPaths, operation: () => Promise<T>): Promise<T> {
  const lockPath = path.join(paths.runtimeDir, "command-store.lock");
  ...
  while (true) {
    try {
      ...
      if (Date.now() - startedAt >= TIMEOUT_MS) {
        throw new CraigError("OPERATION_TIMEOUT", "Timed out waiting for the command store lock.", {
          retryable: true,
        });
      }
