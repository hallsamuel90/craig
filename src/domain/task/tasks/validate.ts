import type { ProjectTaskRepoTarget, RunnerSession, TaskPR, TaskPullRequest, TaskPullRequestComment, TaskPullRequestReviewDecision, TaskRecord, TaskPtyTabRecord } from "../types.js";
...
export const normalizeLegacyTaskRecord = (value: unknown): unknown => {
...
const buildLegacyRunnerSession = (candidate: Partial<TaskRecord>): RunnerSession | null => {
...
const normalizeProjectRepoTargets = (value: unknown): ProjectTaskRepoTarget[] | undefined => {
...
const isProjectRepoTargets = (value: unknown): value is ProjectTaskRepoTarget[] => {
  return Array.isArray(value) && value.every(isProjectRepoTarget);
};

const isProjectRepoTarget = (value: unknown): value is ProjectTaskRepoTarget => {
  if (typeof value !== "object" || value === null) {
    return false;
  }
...
const normalizeTaskPtyTabs = (candidate: Partial<TaskRecord>): TaskPtyTabRecord[] => {
...
const buildDefaultTaskPtyTabs = (taskId: string, _prompt: string, timestamp: string, runner: RunnerType): TaskPtyTabRecord[] => {
...
const normalizeRunner = (value: string | null | undefined): RunnerType => {
  return value && configService.runners.isRunnerType(value) ? value : "codex";
};

const isTaskPtyTabs = (value: TaskRecord["ptyTabs"] | undefined): value is TaskPtyTabRecord[] => {
  return (
    Array.isArray(value) &&
    value.every(
      (tab) =>
...
const isRunnerSession = (value: TaskRecord["runnerSession"] | undefined): boolean => {
  return (
    typeof value === "object" &&
    value !== null &&
...
const isPromptSource = (value: TaskRecord["prompt"] | undefined): boolean => {
  return (
    typeof value === "object" &&
    value !== null &&
...
const isChecks = (value: TaskRecord["checks"] | undefined): boolean => {
  return (
    typeof value === "object" &&
    value !== null &&
...
const isLastCommit = (value: TaskRecord["lastCommit"] | undefined): boolean => {
  return (
    value === null ||
    (typeof value === "object" &&
      value !== null &&
...
const isPullRequestChecks = (value: unknown): boolean => {
  return (
    Array.isArray(value) &&
    value.every(
      (check) =>
...
const isPrs = (value: unknown): value is TaskPR[] => {
  return Array.isArray(value) && value.every(isPullRequest);
};

const isArtifacts = (value: TaskRecord["artifacts"] | undefined): boolean => {
  return (
    typeof value === "object" &&
    value !== null &&
...
const isCleanup = (value: TaskRecord["cleanup"] | undefined): boolean => {
  return (
    typeof value === "object" &&
    value !== null &&
...
const normalizeLegacyChecks = (candidate: Partial<TaskRecord>): TaskRecord["checks"] => {
  if (!candidate.checks) {
    return {
      source: {
        type: "repo_config",
...
const normalizeLegacyPrs = (candidate: Partial<TaskRecord> & { pullRequest?: TaskPullRequest }): TaskPR[] => {
...
const normalizeLegacyPr = (value: unknown): TaskPR => {
...
const normalizeLegacyPrStatus = (status: string | null | undefined): TaskPR["status"] => {
...
const normalizeLegacyReviewDecision = (value: unknown): TaskPullRequestReviewDecision => {
...
const normalizeLegacyPrComments = (value: unknown): TaskPullRequestComment[] => {
  if (!Array.isArray(value)) {
    return [];
  }
...
const normalizeLegacyArtifacts = (candidate: Partial<TaskRecord>): TaskRecord["artifacts"] => {
