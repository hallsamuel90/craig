...
export class Heartbeat {
  ...
  start(): void {
    ...
    for (const job of this.jobs.values()) job.nextRunAt = startedAt + job.intervalMs;
  ...
  private tick(): void {
    ...
    for (const job of this.jobs.values()) {
      if (job.inFlight || job.nextRunAt === null || currentTime < job.nextRunAt) continue;
      job.inFlight = true;
      job.nextRunAt = currentTime + job.intervalMs;
      void Promise.resolve()
        .then(() => job.run(signal))
        .catch((error: unknown) => {
          if (!signal.aborted) return Promise.resolve(this.onError(job.id, error)).catch(() => undefined);
          return undefined;
        })
        .finally(() => {
          job.inFlight = false;
