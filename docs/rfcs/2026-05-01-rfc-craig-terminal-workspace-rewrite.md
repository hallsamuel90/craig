...
- `3.4` Add a background daemon that preserves live PTY sessions across Craig UI exits and restarts: `implemented, delegated-session ownership and live task invalidation hardened, legacy tmux substrate removed, and verified`
...
- `5.1` Add Cursor and Claude runner support alongside Codex: `implemented, daemon-only launch hardened, and verified`
...
- `3.4` Verified by adding a workspace-local Craig PTY daemon with hidden serve/shutdown CLI paths, short hashed Unix-socket IPC, `.craig/runtime` pid/log metadata, stale endpoint recovery, and a daemon-backed PTY client that preserves the existing app PTY port while moving live `node-pty` and xterm emulator ownership out of the foreground UI process. Foreground Craig startup now connects to the daemon before accepting shell input; foreground exit detaches from daemon sessions instead of killing them; explicit PTY tab close still terminates only the closed daemon session; and task-scoped `agent`/`terminal` tab ids continue to drive session identity and worktree/command resolution. Root, delegated, and Fury task creation now provision through the domain boundary and launch through a shell-owned daemon port, so initial prompts, unselected dispatch, and later TUI navigation all address the same durable agent tab and process. Coalesced task lifecycle events now flow through the existing daemon client invalidation channel, allowing an open TUI to load and render tasks created by another process without polling or user input. The legacy tmux adapters, session records, task attach/focus commands, task-session ids, pane cleanup state, test stubs, and runtime directory creation were removed; supported legacy task records are normalized by stripping the retired fields. Automated verification covers daemon reconnect without respawn, explicit per-tab disposal, stale pid recovery, app attach/close behavior, real terminal restart reattach with a stub Codex process proving the agent was not relaunched, delegated Cursor launch/dispatch/navigation identity, daemon lifecycle invalidation, targeted TUI child insertion, and legacy record migration. The current full suite passed with 662 tests across 66 files via `pnpm test`, plus `pnpm typecheck`, `pnpm lint`, `pnpm build`, `pnpm build:npm`, `pnpm package:audit`, and `pnpm package:smoke`.
...
Resume at manual verification of `5.2`. All implementation items are complete, delegated child session identity and live task invalidation are covered automatically under `3.4`, and Craig no longer requires or invokes tmux; no separate daemon follow-up is required unless the Fury bug bash exposes a regression. During the manual pass, keep the parent TUI open while an agent creates children and confirm each row appears without navigation or refresh. Project-task Review action dispatch is intentionally out of scope — PR create/sync, refresh checks, and merge for project tasks are agent-owned, not Craig-dispatched. The remaining work before marking `5.2` verified is to run the manual verification steps in the `5.2` handoff against a real project workspace. Phase `7.1` was completed out of order and should be skipped on future resumes unless release packaging regresses.
