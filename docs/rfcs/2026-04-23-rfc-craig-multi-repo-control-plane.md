...
- `1.1` Workspace bootstrap, archive model, and repo registry: `pending`
...
- `1.1` Not yet verified under this superseding RFC. Prior repo-local bootstrap behavior exists in the codebase but does not satisfy the workspace-scoped repo-registry model defined here.
...
Resume at the first sub-phase that is not both implemented and verified. The current resume point is `1.1`.
