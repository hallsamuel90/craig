...
Resume at `4.1`. Treat the verified Phase `3.1` durable command state machine, single-leader daemon supervisor, target-specific delivery port, idempotency rules, command event reconciliation, and uncertain-delivery handling as the execution boundary for capability-scoped parent/child delegation. Add capability and lineage policy in the domain without allowing agent status, PTY transport, or CLI parsing concerns to leak into orchestration rules.
