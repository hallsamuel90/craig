...
Resume at `5.1`. Treat the verified Phase `4.1` capability, actor, lineage, bounded delegation, idempotency, and tree-cancellation contracts as the only mutation boundary available to future swarm execution. Add strict YAML parsing, validation, and side-effect-free DAG planning without creating tasks, capabilities, command records, events, or run state during validation and planning.
