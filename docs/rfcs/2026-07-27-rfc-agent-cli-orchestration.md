...
Resume at `3.1`. Treat the verified Phase `2.2` generic event schema, monotonic workspace journal, cursor/replay rules, semantic reconciliation, and JSONL watch contract as the durable backbone for prompt command persistence and inspection. Reuse preview-enabled cursor-backed waiting, keep prompt delivery explicitly targeted by task and tab, and preserve the rule that uncertain delivery is never silently replayed.
