...
- `3.1` Add durable, target-specific prompt dispatch and command inspection/wait/cancel: `implemented, review-hardened, runner-submission and daemon-only launch hardened, and verified`
- `4.1` Add capability-scoped parent/child delegation and tree cancellation: `implemented, project-scope, initial-session, and daemon-only launch hardened, and verified`
...
- Current daemon-only task launch, live TUI invalidation, and legacy-record migration hardening passed with 662 tests across 66 files via `pnpm test`, plus `pnpm typecheck`, `pnpm lint`, `pnpm build`, `pnpm build:npm`, `pnpm package:audit`, and `pnpm package:smoke`.
